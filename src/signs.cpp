...
// PROGMEM font data to save RAM and prevent fragmentation

// System font data - 157 characters starting from ASCII 32
const PROGMEM int font_system_data[] = {
    // Each character is 7 bytes (7 rows of pixel data)
    ...
    0x00, 0x80, 0x40, 0x20, 0x10, 0x08, 0x00, // 92 - backslash
...
// Bold number font data - 10 characters (0-9) starting from ASCII 48
const PROGMEM int font_bold_data[] = {
    // Each character is 7 bytes (7 rows of pixel data)
...
const font font_system = {
    "system",               // name
    5,                      // sizeX
    7,                      // sizeY  
    32,                     // offset (ASCII 32)
    font_system_data,       // data pointer to PROGMEM
    157                     // dataSize (number of characters)
};
...
const font font_bold = {
    ...
    7,                     // sizeY
    48,                    // offset (ASCII 48)
    font_bold_data,        // data pointer to PROGMEM
    10                     // dataSize (number of characters)
};

// Helper functions for PROGMEM font access
const font* getFontById(uint8_t fontid) {
    switch(fontid) {
        case 0: return &font_system;
        case 1: return &font_bold;
        default: return &font_system; // fallback to system font
    }
}

const font* getFontByName(const char* name) {
    if (strcmp(name, "system") == 0) return &font_system;
    if (strcmp(name, "boldnumber") == 0) return &font_bold;
    return &font_system; // fallback to system font
}

std::vector<int> getCharacterData(const font& font, int charIndex) {
    std::vector<int> charData;
    charData.reserve(font.sizeY); // Each character has sizeY rows
    
    int startIndex = charIndex * font.sizeY;
    for (int i = 0; i < font.sizeY; i++) {
        charData.push_back(pgm_read_word(&font.data[startIndex + i]));
    }
